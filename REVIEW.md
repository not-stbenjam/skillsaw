...
would substantially increase a rule's complexity. Skillsaw supports rule
disabling, scoped suppressions, and global or per-rule exclusion patterns
that can exclude whole groups of files at once. Accept these mitigations when
they are practical for the affected use. Flag a case only when its likelihood or
